...
CRDT::generatePosBetween(std::vector<Identifier> pos1, std::vector<Identifier> pos2, QString siteId,
						 std::vector<Identifier> newPos, int level) {
...
void CRDT::insertChar(Character character, Pos pos) {
	...
	if (character.getValue() == '\n') {
		...
		if (!lineAfter.empty()) {
			...
			this->style.insert(this->style.begin() + pos.getLine() + 1,
...
std::vector<Character> CRDT::deleteMultipleLines(Pos startPos, Pos endPos) {
	...
	if (!structure[endPos.getLine()].empty()) {
		chars.insert(chars.end(), structure[endPos.getLine()].begin(),
					 structure[endPos.getLine()].begin() + endPos.getCh());
		structure[endPos.getLine()].erase(structure[endPos.getLine()].begin(),
										  structure[endPos.getLine()].begin() + endPos.getCh());
	}
			...
			if (character.getSiteId() == getSiteId()) {
				...
				if (copy) {
					...
					if (!waitForInvalidate) {
						...
						editor->insertChar(character.getValue(), character.getTextCharFormat(), pos,
										   message.getSender(), character);
					}
				}
			}
...
void CRDT::insertBlock(Character character, Pos position) {
	if (position.getCh() == 0) {
		...
		this->style.insert(
				this->style.begin() + position.getLine(), std::pair<Character, int>(
						generateChar('-', character.getTextCharFormat(), blockPos, character.getSiteId()),
						0x4));
	}
	...
	if (character.getValue() == '\n') {
		...
		this->style.insert(
				this->style.begin() + position.getLine(), std::pair<Character, int>(
						generateChar('-', character.getTextCharFormat(), blockPos, character.getSiteId()), 0x4));
	}
}
